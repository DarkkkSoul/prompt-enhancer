    ...
    return (
        <div className='w-1/2 h-full mx-auto p-10 gap-y-10 flex flex-col items-center justify-between'>
            ...
            <div className='flex-1 w-full flex flex-col items-center justify-center gap-8'>
                ...
                <div className='w-full max-w-4xl flex items-center gap-5 bg-[#305A6C]/40 px-4 py-5 border-5 border-[#FFFFFF]/40 rounded-xl'>
                    <textarea
                        type="text"
                        value={userPrompt}
                        ...
                        className='flex-1 bg-[#B5DCED]/40 rounded-xl py-3 px-4 text-white placeholder:text-white/60 resize-none h-20 shadow-inner shadow-[#1f3d4a]/40 hind-400 tracking-wide'
                    />
                    <button
                        onClick={handleGenerate}
                        disabled={isLoading || !userPrompt.trim()}
                        className={`transition-all hover:scale-[1.03] border-3 border-[#FFFFFF]/40 bg-linear-350 from-[#31746c] via-[#4fa59b] via-60% via-[#31746c] to-[#FFFFFF]/30 to-90% rounded-xl py-2 px-5 kant-400 text-white tracking-wide shadow-[0_6px_18px_rgba(31,61,74,0.45)] cursor-pointer ${isLoading || !userPrompt.trim() ? 'opacity-60 cursor-not-allowed' : ''}`}
                    >
